...
class SwissEph {
  ...
  rise_trans(julianDay, planet, longitude, latitude, altitude, flags, epheFlag = this.SEFLG_SWIEPH) {
    const resultPtr = this.SweModule._malloc(10 * Float64Array.BYTES_PER_ELEMENT);
    const geoPosPtr = this.SweModule._malloc(3 * Float64Array.BYTES_PER_ELEMENT);
    const serrPtr = this.SweModule._malloc(256);
    this.SweModule.HEAPF64[geoPosPtr >> 3] = longitude;
    this.SweModule.HEAPF64[(geoPosPtr >> 3) + 1] = latitude;
    this.SweModule.HEAPF64[(geoPosPtr >> 3) + 2] = altitude || 0;
    const retFlag = this.SweModule.ccall(
      'swe_rise_trans',
      'number',
      ['number', 'number', 'pointer', 'number', 'number', 'pointer', 'number', 'number', 'pointer', 'pointer'],
      [julianDay, planet, 0, epheFlag, flags, geoPosPtr, 0, 0, resultPtr, serrPtr]
    );
    const results = new Float64Array(this.SweModule.HEAPF64.buffer, resultPtr, 10).slice();
    this.SweModule._free(serrPtr);
    this.SweModule._free(geoPosPtr);
    this.SweModule._free(resultPtr);
    return retFlag < 0 ? null : results;
  ...
  rise_trans_true_hor(julianDay, planet, longitude, latitude, altitude, flags, horizon = -0.833, epheFlag = this.SEFLG_SWIEPH) {
    const resultPtr = this.SweModule._malloc(10 * Float64Array.BYTES_PER_ELEMENT);
    const geoPosPtr = this.SweModule._malloc(3 * Float64Array.BYTES_PER_ELEMENT);
    const serrPtr = this.SweModule._malloc(256);
    this.SweModule.HEAPF64[geoPosPtr >> 3] = longitude;
    ...
    const retFlag = this.SweModule.ccall(
      ...
      [julianDay, planet, 0, epheFlag, flags, geoPosPtr, 0, 0, horizon, resultPtr, serrPtr]
    );
    const results = new Float64Array(this.SweModule.HEAPF64.buffer, resultPtr, 10).slice();
    this.SweModule._free(serrPtr);
    this.SweModule._free(geoPosPtr);
