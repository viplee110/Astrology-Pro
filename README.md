...
- 隐私友好分享：主动生成 URL 分享链接，编码本命资料和计算设置，不创建服务器档案。
...
- 分享链接：复制当前本命盘的 URL 片段，方便在其他浏览器或设备打开同一张盘。
...
& "C:\Users\viplee\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe" .\scripts\check-static-pages.mjs
```

这些脚本会检查默认样例的 UTC 换算、Swiss Ephemeris 初始化、核心星体位置、宫位、相位、虚点、Markdown 与 AI Prompt 生成，并将包装层结果与直接 Swiss Ephemeris 调用结果比对；同时检查静态页面、canonical、sitemap 和本地链接是否完整。

GitHub Actions 会在 `main` 分支 push 和 pull request 时自动运行同一套回归和静态页面检查。
